...
@click.argument('mode', type=click.Choice(['bcall', 'support']))
@click.option('--input', '-i', default = ".", required=True, help='Input; a singular, indexed bam file. ')
@click.option('--output', '-o', default="maegtk_out", help='Output directory for genotypes.')
@click.option('--name', '-n', default="maegtk",  help='Prefix for project name')
...
@click.option('--barcode-tag', '-bt', default = "CB",  help='Read tag (generally two letters) to separate single cells; valid and required only in `bcall` mode.')
@click.option('--barcodes', '-b', default = "",  help='File path to barcodes that will be extracted; useful only in `bcall` mode.')
@click.option('--min-barcode-reads', '-mb', default = 100,  help='Minimum number of mitochondrial reads for a barcode to be genotyped; useful only in `bcall` mode; will not overwrite the `--barcodes` logic.')
...
@click.option('--nsamples', '-ns', default = 6000, help='The number of samples / cells to be processed per iteration; default is all.')
...
def main(mode, input, output, name, mito_genome, ncores,
	...
	"""
	maegtk: a Maester genome toolkit. \n
	MODE = ['bcall', 'support'] \n
	"""
	
	script_dir = os.path.dirname(os.path.realpath(__file__))
	cwd = os.getcwd()
	__version__ = get_distribution('maegtk').version
	click.echo(gettime() + "maegtk v%s" % __version__)
	...
	if(mode == "support"):
		click.echo(gettime() + "List of built-in genomes supported in maegtk:")
	...
	if(mode == "check" or mode == "call"):
		...
		nsamplesNote = "maegtk will process " + str(len(samples)) + " samples"
		
	if(mode == "check"):
		# Exit gracefully
		sys.exit(gettime() + "maegtk check passed! "+nsamplesNote+" if same parameters are run in `call` mode")	
	...
	if(mode == "call" or mode == "one"):
		...
		if(mode == "call"):
			# Logging		
			logf = open(output + "/logs" + "/base.maegtk.log", 'a')
			click.echo(gettime() + "Starting analysis with maegtk", logf)
		...
		if(mode == "call"):
			...
			snakecmd_scatter = 'snakemake'+snakeclust+' --snakefile ' + script_dir + '/bin/snake/Snakefile.maegtk.Scatter --cores '+ncores+' --config cfp="'  + y_s + '" --stats '+snake_stats + snake_log_out
			os.system(snakecmd_scatter)
			click.echo(gettime() + "maegtk successfully processed the supplied .bam files", logf)
	...
	if(mode == "gather" or mode == "call"):
		...
		if(mode == "call"):
			maegtk_directory = output
			
		elif(mode == "gather"): # in gather, the input argument specifies where things are
			logf = open(input + "/logs" + "/base.maegtk.log", 'a')
			click.echo(gettime() + "Gathering samples that were pre-called with `one`.", logf)
			maegtk_directory = input
			
		dict2 = {'maegtk_directory' : sqs(maegtk_directory), 'name' : sqs(name), 'script_dir' : sqs(script_dir)}
		y_g = maegtk_directory + "/.internal/parseltongue/snake.gather.yaml"
		...
		snakecmd_gather = 'snakemake --snakefile ' + script_dir + '/bin/snake/Snakefile.maegtk.Gather --cores '+ncores+' --config cfp="' + y_g + '" --stats '+snake_stats + snake_log_out
		os.system(snakecmd_gather)
		
		# Make .rds file from the output
		Rcall = "Rscript " + script_dir + "/bin/R/toRDS.R " + maegtk_directory + "/final " + name
		os.system(Rcall)
